...
if(isset($_POST['register'])){ // registration process 
    ...
    if(strlen($_POST['name'])<3){
        ...
        //creating user registration in a database 
        $query = "insert into users (name,email,password,token) values('$name','$email','$password','$token')";
        
        $db->query($query);
       // ini_set("SMTP", "aspmx.l.google.com");
       // ini_set("sendmail_from", "aleksandrskucinskis@gmail.com");
        $message = "Hi $name! Account created here is the activation link http://localhost/website/registration/activate.php?token=$token";
      // email to user to click the registration link 
        mail($email , 'Activate Account' , $message , 'From: aleksandrskucinskis@gmail.com');
